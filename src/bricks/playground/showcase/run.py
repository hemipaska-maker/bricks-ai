...
Usage:
    python -m bricks.playground.showcase.run --live                          # all scenarios, default model
    python -m bricks.playground.showcase.run --live --scenario CRM-pipeline
    python -m bricks.playground.showcase.run --live --scenario TICKET-pipeline
    python -m bricks.playground.showcase.run --live --model gpt-4o-mini      # OpenAI
    python -m bricks.playground.showcase.run --live --model gemini/gemini-2.0-flash
    python -m bricks.playground.showcase.run --live --model ollama/llama3    # local, no API key
    python -m bricks.playground.showcase.run --live --model claudecode       # ClaudeCode (claude -p), both engines
"""
...
from bricks.playground.showcase.formatters import print_cost_summary
from bricks.playground.showcase.metadata import make_run_dir, write_metadata

_run_logger = logging.getLogger("bricks.playground.showcase.run")
...
def run_benchmark(
    ...
    Args:
        ...
        model: Model string — ``'claudecode'`` or a LiteLLM model string.
    """
    from bricks.playground.showcase.crm_scenario import (
        run_crm_hallucination,
        run_crm_pipeline,
        run_crm_reuse,
    )
    from bricks.playground.showcase.engine import BricksEngine, RawLLMEngine
    from bricks.playground.showcase.ticket_scenario import run_ticket_pipeline
...
def _setup_logger(run_dir: Path) -> logging.Logger:
    ...
    for name in ("bricks", "bricks.providers.claudecode", "bricks.playground"):
...
def _run_custom_scenario(yaml_path: str, model: str = DEFAULT_MODEL) -> None:
    ...
    from bricks.playground.web.scenario_loader import _resolve_raw_data, load_scenario
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python -m bricks.playground.showcase.run --live --model claudecode\n"
        ),
    )
    parser.add_argument(
        "--scenario",
    ...
    if not args.live:
        ...
        print()
        print("Usage:")
        print("  python -m bricks.playground.showcase.run --live")
        print("  python -m bricks.playground.showcase.run --live --model gpt-4o-mini")
        print("  python -m bricks.playground.showcase.run --live --model claudecode")
        print()
